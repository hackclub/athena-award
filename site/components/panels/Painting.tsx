...
export default function Painting({image, index, tooltip, description, descriptionBottom, link, className, showCaptionOnSmall=false}: {image: string, index?: string, description: string, descriptionBottom?: string, link?: string, className?: string, tooltip?: string, showCaptionOnSmall?: boolean}){
    return (
        <>
        ...
        <div data-tooltip-id={index} data-tooltip-content={tooltip} className={`h-42 sm:h-80 flex flex-col items-center justify-center ${className}`}> {/* maybe add small random rotations? */}
            <a target="_blank" className = "text-black no-underline">
                <span className = {`${showCaptionOnSmall ? "inline" : "hidden sm:inline"} text-sm sm:text-base mx-auto w-max max-w-full px-5 my-4 py-2 ${shineEffect("bg-[#F4BF4F] border-[#F4BF4F]/80 rounded-sm text-gray-600")}`}>
