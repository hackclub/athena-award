...
import { ArrowTopRightOnSquareIcon, CodeBracketIcon, ChatBubbleLeftRightIcon } from '@heroicons/react/24/outline';

const hcPrimaryIcon = new L.Icon({
  iconUrl: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="32" height="48" viewBox="0 0 32 48"><path fill="%23e33750" stroke="white" stroke-width="2" d="M16 2C8.268 2 2 8.268 2 16c0 9.941 12.5 28 14 28s14-18.059 14-28C30 8.268 23.732 2 16 2z"/><circle fill="white" cx="16" cy="16" r="6"/></svg>',
  iconSize: [32, 48],
  iconAnchor: [16, 48],
  popupAnchor: [0, -48],
...
export default function MapView() {
  ...
  const fetchEmojiForProject = async (projectName: string) => {
    try {
      const res = await fetch('https://ai.hackclub.com/chat/completions', {
        ...
        body: JSON.stringify({
          messages: [{ role: 'user', content: `Convert the project name "${projectName}" into a single emoji that best represents it. Only output the emoji, nothing else.` }]
        })
      });
  ...
  const selectedProject = selectedProjectIdx !== null ? coordinates[selectedProjectIdx] : null;
  const [mosaicEmoji, setMosaicEmoji] = useState<string | null>(null);

  useEffect(() => {
    if (selectedProject && selectedProject.label && selectedProject.label[0]?.project_name) {
      fetchEmojiForProject(selectedProject.label[0].project_name).then((emoji) => {
  ...
  const mosaicEmojiStyle = useCallback((x: number, y: number, delay: number): React.CSSProperties => ({
    position: 'absolute',
        ...
        <div className="rounded-2xl z-0 h-full shadow-lg w-full border-4 border-white/20 bg-white/10 flex flex-col md:flex-row">
          <div className="w-full h-full col-span-1">
            { isLoading ? <div className = "flex items-center justify-center text-white">Loading projects... 🌎</div> : 
              ...
              >
                {selectedProject && (
                  <MapFocusHandler lat={selectedProject.lat} lng={selectedProject.long} />
                )}
                <TileLayer
                ...
                {coordinates.map((coord: any, idx: number) => (
                  <Marker key={idx} position={[coord.lat, coord.long]} icon={hcPrimaryIcon}>
                    <Popup>
                        ...
                        {coord.label && Array.isArray(coord.label) && coord.label.length > 0 ? (
                          <>
                            ...
                            {coord.label[0].playable_url && (
                              <div>
                                <a href={coord.label[0].playable_url} target="_blank" rel="noopener noreferrer" className="underline !text-white !hover:text-hc-blue transition font-semibold uppercase flex items-center gap-2">
                            ...
                            {coord.label[0].code_url && (
                              <div>
                                <a href={coord.label[0].code_url} target="_blank" rel="noopener noreferrer" className="underline !text-white !hover:text-hc-blue transition font-semibold uppercase flex items-center gap-2">
                  ...
                  {coordinates.map((coord: any, idx: number) => (
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-1 flex-1">
                        {coord.label?.[0]?.playable_url && (
                          <a href={coord.label[0].playable_url} target="_blank" rel="noopener noreferrer" className="!text-white !hover:text-hc-blue underline font-semibold flex items-center gap-2">
                            <ArrowTopRightOnSquareIcon className="w-5 h-5 inline-block" />
                            Try it out
                          </a>
                        )}
                        {coord.label?.[0]?.code_url && (
                          <a href={coord.label[0].code_url} target="_blank" rel="noopener noreferrer" className="!text-white !hover:text-hc-blue underline font-semibold flex items-center gap-2">
                            <CodeBracketIcon className="w-5 h-5 inline-block" />
                            Source Code
                          </a>
                        )}
